...
for(sim_iteration in 1:nrsims){
  ...
  mod_deep <- deepregression(y = train$accel, 
                             list_of_formulas= list(loc = ~ 0 + d(times),
                                                     scale = ~ 1 + times),
                             list_of_deep_models = list(d = deep_mod, NULL),
                             data = train,
                             family = "normal"
                             )
  ...
  if(!is.nan(rmse) && best_rmse > rmse){
    ...
    q40 <- mod_deep %>% quant(data = mcycle, value = 0.4)
    q60 <- mod_deep %>% quant(data = mcycle, value = 0.6)
    q10 <- mod_deep %>% quant(data = mcycle, value = 0.1)
    q90 <- mod_deep %>% quant(data = mcycle, value = 0.9)
